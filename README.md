...
1. Open **`http://<box-ip>:8080/onboarding`** — answer two questions (what kind of place, how
   ...
   each handset (by MAC) with the extension it should own; and SSL stays **off by default** —
   LAN phones reject a self-signed cert, so provisioning runs over plain HTTP unless you have a
   CA-signed certificate (media is SRTP-encrypted either way). One click applies it.
